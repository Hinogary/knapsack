...
fn main() {
    ...
    let input = fs::read_to_string(opts.input_task.clone()).expect("Failed to read input_task file!");
    ...
    let durations = input.lines()
        .map(parse_problem_line)
        .map(|problem|{
            let start = Instant::now();
            (match (solver, problem.min_cost.is_none() || opts.force_construction){
            ...
            if let Some(items) = &solution.items {
                output += items.iter().map(|&i| if i {" 1"} else {" 0"}).join("").as_str();
            }
            println!("time: {:?} visits: {} p_visits: {}\n{}",
                elapsed,
                solution.visits.unwrap(),
                solution.parent_visits.unwrap(),
                output);
            if let Some(reference) = ref_solutions.as_ref().unwrap_or(&HashMap::new()).get(&solution.id){
...
fn next_parse<'a, T, K>(iter: &mut T) -> K
    where T: Iterator<Item = &'a str>, K: FromStr, <K as std::str::FromStr>::Err: std::fmt::Debug
{
    iter.next().unwrap().parse().unwrap()
}

fn parse_problem_line(line: &str) -> Problem {
    let mut iter = line.split(' ').filter(|x| x.len() > 0);
    ...
    assert_eq!(iter.next(), None, "Line was not exhausted, wrong problem line!");
    Problem{
        id: id.abs() as u32,
        max_weight, size, min_cost, items,
    }
}
...
fn parse_solution_line(line: &str) -> Solution {
    ...
    let items = Some((0..size)
        .map(|_|match iter.next().unwrap() {
            "1" => true,
            "0" => false,
            _ => panic!("Reference solution is not in (0, 1)!"),
        }).collect::<Vec<_>>());
    assert_eq!(iter.next(), None, "Line was not exhausted, wrong solution line!");
    Solution{
        id, size, cost, items,
        visits: None, parent_visits: None,
    }
}

fn construction_naive(problem: &Problem) -> Solution {

    fn rec_fn(problem: &mut ProblemWithSol, cost: u32, weight: u32, index: usize, best_cost: u32) -> u32 /* best_cost */ {
        if index < problem.p.size {
            ...
            let best_without_item = rec_fn(
                problem, cost, weight,
                index + 1, best_cost.max(best_with_item));
...
fn decision_naive(problem: &Problem) -> Solution {
    ...
    fn rec_fn(problem: &mut ProblemWithSol, cost: u32, weight: u32, index: usize, min_cost: u32) -> u32 /* best_cost */ {
        if index < problem.p.size {
            problem.parent_visits += 1;
            let best_with_item = rec_fn(
                problem,
    ...
    Solution{
        ...
        size: problem.size,
        cost,
        items: if cost > problem.min_cost.unwrap() {Some(aug_problem.best_solution)} else {None},
        visits: Some(aug_problem.visits),
        parent_visits: Some(aug_problem.parent_visits),
    }
}
...
fn construction_pruning(problem: &Problem) -> Solution {
    ...
    fn rec_fn(problem: &mut ProblemWithCostRem, cost: u32, weight: u32, index: usize, best_cost: u32) -> u32 {
        if index < problem.p.size {
            problem.parent_visits += 1;
...
fn decision_pruning(problem: &Problem) -> Solution {
    ...
    fn rec_fn(problem: &mut ProblemWithCostRem, cost: u32, weight: u32, index: usize, min_cost: u32) -> u32 {
        if index < problem.p.size {
            problem.parent_visits += 1;
